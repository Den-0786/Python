...
class QuizUI:
    ...
    def __init__(self, quiz: QuizBrain):
        ...
        self.score_text = self.canvas.create_text(440, 30,
        ...
        # Buttons (True/False)
        true_img = PhotoImage(file="quizzier/true.png")
        self.true_btn = Button(image=true_img, highlightbackground=THEME_COLOR,command=self.click_true)
        self.true_btn.grid(row=3, column=1)

        false_img = PhotoImage(file="quizzier/false.png")
        self.false_btn = Button(image=false_img, highlightbackground=THEME_COLOR,command=self.click_false)
    ...
    def next_qn(self):
        ...
        if self.quiz.still_has_questions():
            ...
            self.title_label.config(text=f"Your final score: {self.quiz.score} / {self.quiz.question_number}"
                )
